...
import { CREATE_SESSION, JOIN_SESSION } from '../graphql/mutations/sessionMutations';
...
const Sessions = () => {
  ...
  const pastSessions = allSessions.filter(s => s.isPast);
        ...
        <div className="flex flex-col lg:flex-row gap-10 w-full items-start">
          ...
          <div className="flex flex-col flex-1 w-full min-w-0 px-4 md:px-8">
            <main className="flex-1 font-worksans max-w-5xl">
              ...
              <p className="text-gray-600 mb-8 text-lg font-medium">{upcomingSessions.length} upcoming · {pastSessions.length} completed</p>

              {/* Tabs */}
              <div className="flex space-x-4 mb-2">
                <button 
                  onClick={() => setActiveTab('upcoming')}
                  className={`px-12 py-3 rounded-xl border transition-colors ${activeTab === 'upcoming' ? 'bg-[#FBE58D] border-[#d6a546] font-medium' : 'bg-white border-gray-300 text-gray-700'}`}
                >
                  Upcoming ({upcomingSessions.length})
                </button>
                <button 
                  onClick={() => setActiveTab('completed')}
                  className={`px-12 py-3 rounded-xl border transition-colors ${activeTab === 'completed' ? 'bg-[#FBE58D] border-[#d6a546] font-medium' : 'bg-white border-gray-300 text-gray-700'}`}
                >
                  Completed ({pastSessions.length})
                </button>
                <button 
                  onClick={() => setActiveTab('all')}
                  className={`px-12 py-3 rounded-xl border transition-colors ${activeTab === 'all' ? 'bg-[#FBE58D] border-[#d6a546] font-medium' : 'bg-white border-gray-300 text-gray-700'}`}
                >
                ...
                {displayData.map((session) => (
                  <div key={session.id} className="border border-gray-300 rounded-[2.5rem] p-8 bg-white hover:shadow-md transition-shadow">
                    ...
                    <div className="flex flex-wrap gap-8 text-gray-600 mb-6 font-medium">
                      ...
                      <button 
                        onClick={() => !session.hasJoined && handleJoin(session.id)}
                        disabled={session.isPast || session.hasJoined}
                      ...
                      >
                        ...
                        {session.isPast ? 'Ended' : session.hasJoined ? 'Joined' : 'Join'}
                      </button>
                    </div>
                  </div>
                ))}
    ...
    try {
      await createSession({
        variables: {
          ...
          location: selectedType === 'VIRTUAL' ? 'Online link will be provided' : 'TBD Location'
        }
      });
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {['30 min', '1 hour', '1.5 hour', '2 hours', '3 hours'].map((dur) => (
                <button
                  key={dur}
                  onClick={() => setSelectedDuration(dur)}
                  className={`px-6 py-2.5 rounded-[2rem] border transition-colors ${
                    selectedDuration === dur ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-400 hover:border-gray-500'
                  }`}
                >
                  {dur}
                </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className={`py-3 rounded-[2rem] border transition-colors ${
                  selectedType === 'VIRTUAL' ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-400 hover:border-gray-500'
                }`}
              >
                online
              </button>
              <button
                ...
                className={`py-3 rounded-[2rem] border transition-colors ${
                  selectedType === 'IN_PERSON' ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-400 hover:border-gray-500'
                }`}
              >
                in-person
              </button>
          ...
          <div className="flex justify-end pt-4">
            <button 
              ...
              className="bg-[#2d2d2d] text-white px-16 py-3.5 rounded-xl hover:bg-black transition-colors font-medium">
              Continue
            </button>
          </div>
        </div>
